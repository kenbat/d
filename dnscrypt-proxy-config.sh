...
wget -N https://raw.githubusercontent.com/kenbat/d/master/blacklist-ips/blacklist-ips.txt
cat blacklist-ips.txt dnscrypt-blacklist-ips.txt | sort | uniq >blocked-ips.txt
sed -e 's/$/\r/' blocked-ips.txt >blocked-ips1.txt
wget -N https://raw.githubusercontent.com/kenbat/d/master/blacklist-ips/rebinding-protection.txt
cat rebinding-protection.txt blocked-ips1.txt > blocked-ipss.txt
mv blocked-ipss.txt blocked-ips.txt

wget -N https://raw.githubusercontent.com/kenbat/d/master/cloaking-rules/cloaking-rule.txt
...
cat accelerated-domains.china.conf | grep -v '^#server' | sed -e 's|/| |g' -e 's|^server= ||' | sed 's/114.114.114.114/167.114.220.125,139.59.200.116,46.227.203.9/g' | sed -e 's/$/\r/' >forwarding-rules.txt
